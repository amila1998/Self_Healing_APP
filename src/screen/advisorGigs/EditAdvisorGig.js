...
const EditAdvisorGig = ({route}) => {
    const { AGID,AGD } = route.params
    console.log("🚀 ~ file: EditAdvisorGig.js ~ line 15 ~ EditAdvisorGig ~ AGD", AGD)
    console.log("🚀 ~ file: EditAdvisorGig.js ~ line 15 ~ EditAdvisorGig ~ AGID", AGID)
    const { profile } = useLogin();
    const [userID, setUserID] = useState('636dec9eadb502154e8c348f');
    const [description, setDescription] = useState(AGD);
    console.log("🚀 ~ file: EditAdvisorGig.js ~ line 20 ~ EditAdvisorGig ~ description", description)
    const Navigator = useNavigation();
    // useEffect(() => {
    //     const getDetails = async () => {
    //         try {
    //             const res = await client.get(`/api/gig/${AGID}`)
    ...
    const editPost = async (e) => {
        ...
        if (description === "") {
            ...
            try {
                const res = await client.put(`/api/advisorGig/update/${AGID}`,{description:description});
                console.log(res)
                alert(res.data.msg);
            } catch (err) {
                console.log(err);
                ...
                <View style={{ backgroundColor: "rgba(255, 255, 255, 0.5)", borderRadius: 10, margin: 10 }}>
                    <View style={[styles.container, {
                        ...
                        <View style={{ flex: 2, backgroundColor: "" }}>
                            <TextInput  name="description"  style={styles.textInput} onChangeText={setDescription} placeholder='type your idea' value={description}></TextInput>
                        </View>
                    ...
                    <View style={[styles.container, {
                        ...
                        <View style={{ flex: 2, backgroundColor: "", alignItems: 'center' }}>
                            <TouchableOpacity onPress={editPost}>
                                <Text style={{ color: '#fcbb08' }}>Update</Text>
                            </TouchableOpacity>
                        </View>
