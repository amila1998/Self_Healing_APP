...
const EditBusiness = ({route}) => {
  ...
  console.log("🚀 ~ file: EditBusiness.js ~ line 23 ~ EditBusiness ~ status", status)
  const [category, setcategory] = useState('')
  const [description, setdescription] = useState('')
  const { token,setCallBack,callback } = useLogin()
  ...
  useEffect(() => {
    const getBusinessDetails = async () => {
      try {
        ...
        setSelectedItems(res.data.expectations)
  ...
  const updateBusiness = async (e) => {
    ...
    try {
      setCallBack(true)
      const res = await client.patch(`/api/business/update/${BID}`, { business_name, location, status, category, expectations: selectedItems,description }, {
          ...
          <View>
            ...
            <View style={styles.loginBody}>
              <View style={styles.formInput}>
                <TextInput value={business_name} onChangeText={setbusiness_name} style={styles.textInput} placeholder='Business Name'></TextInput>
              </View>
              <View style={styles.formInput}>
                <TextInput value={location} onChangeText={setlocation} style={styles.textInput} placeholder='Location'></TextInput>
              </View>
              <View style={styles.formInput}>
                <SelectList defaultOption={{key:status,value:status==='Already_started'?'Already Started (one month ago)':status==='started_as_new_business'?'Started as a new business':status==='hope_to_start'&&'Hope to start'}} setSelected={setstatus} search={false} dropdownStyles={{ backgroundColor: '#fff' }} boxStyles={styles.textInput} data={BusinessStatusData} placeholder='Business Status' />
              </View>
              <View style={styles.formInput}>
                <SelectList defaultOption={{key:category,value:category}} setSelected={setcategory} search={true} dropdownStyles={{ backgroundColor: '#fff' }} boxStyles={styles.textInput} data={BusinessCategoriesData} placeholder='Business Category' />
              </View>
              ...
              <View style={styles.formInput}>
                <TextInput value={description} onChangeText={setdescription} style={styles.textInput} placeholder='Business Description' numberOfLines={4} multiline={true}></TextInput>
              </View>
              <View style={styles.formInput}>
                <TouchableOpacity onPress={updateBusiness} style={styles.defaultButton}>
